...
//	printf("Serializing for player %u\n", slot);
	...
	*p = players[slot].status; p++;
	...
	//	printf("Writing ID as %u, status=%u\n", players[slot].id, players[slot].status);
		w_int16(p, players[slot].u->id); p += 2;
		// seat
		*p = players[slot].seat; p ++;
		// health
		*p = players[slot].u->hp; p ++;
		// ammo
		*p = players[slot].u->hp; p ++;
...
*/	}
	...
	*o = 0;
	...
	while (u != NULL) {
		...
		*p = (u->team << 7) | u->o->type;
		...
		w_int24(p, (((unsigned int)u->prev_x << 12) | ((unsigned int)u->prev_y) & 0xFFF)); p += 3;

		// speed max is +-64 (7 bits)
		// angle in 512 steps (9 bits)
		w_int16(p, (u->speed << 9) |
				   snap(u->angle, 512)); p += 2;
	...
	*o = 0;
	...
	while (j != NULL) {
		...
		// object x, y
		w_int24(p, (((unsigned int)j->prev_x << 12) | ((unsigned int)j->prev_y) & 0xFFF)); p += 3;
	...
	*o = 0;
	...
	while (f != NULL) {
		...
		*p = f->type; p ++;
		*p = (unsigned char)(f->delay * 256.0); p ++;
